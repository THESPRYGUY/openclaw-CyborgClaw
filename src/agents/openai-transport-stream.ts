...
function resolveOpenAIReasoningEffortForTransportModel(params: {
  model: { provider?: unknown; id?: unknown; compat?: unknown };
  effort: string;
  fallbackMap?: Record<string, string>;
}): OpenAIApiReasoningEffort | undefined {
  const resolved = resolveOpenAIReasoningEffortForModel(params);
  if (!resolved || resolved === "none" || !isMediumOnlyOpenAICodexReasoningModel(params.model)) {
    return resolved;
  }
  return "medium";
}
...
function raiseMinimalReasoningForResponsesWebSearch(params: {
  ...
  for (const effort of ["low", "medium", "high"] as const) {
    const resolved = resolveOpenAIReasoningEffortForTransportModel({
      model: params.model,
      effort,
    });
...
export function buildOpenAIResponsesParams(
  ...
  if (model.reasoning) {
    if (options?.reasoningEffort || options?.reasoning || options?.reasoningSummary) {
      ...
      const resolvedReasoningEffort = resolveOpenAIReasoningEffortForTransportModel({
        model,
      ...
      const reasoningEffort = resolveOpenAIReasoningEffortForTransportModel({
        model,
        effort: "none",
      });
      if (reasoningEffort) {
...
export function buildOpenAICompletionsParams(
  ...
  const resolvedCompletionsReasoningEffort = completionsReasoningEffort
    ? resolveOpenAIReasoningEffortForTransportModel({
        model,
