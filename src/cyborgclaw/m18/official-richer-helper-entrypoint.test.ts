...
describe("M18 official richer-helper entrypoint", () => {
  it("runs prep plus harness for the clean preserved-style source folder and emits a clean bundle", async () => {
    ...
    await expect(
      fs.readFile(path.join(bundleDir, "approval-checkpoint.json"), "utf8"),
  ...
  it("emits the full expected artifact set through the entrypoint path", async () => {
    ...
    await expect(
      fs.readFile(path.join(preparedDir, "approval-checkpoint.json"), "utf8"),
    ).resolves.toContain('"checkpointId"');
    await expect(
      fs.readFile(path.join(preparedDir, "PK-L01.parent.delta.jsonl"), "utf8"),
    ).resolves.toContain("sessions_spawn");
    await expect(
      fs.readFile(path.join(preparedDir, "PK-L01.child.transcript.jsonl"), "utf8"),
    ).resolves.toContain('"id":"0fda66f8-ed45-4755-ba51-24a92bc1785e"');
