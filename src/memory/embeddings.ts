...
function formatLocalSetupError(err: unknown): string {
  ...
  return [
    ...
    missing
      ? "Reason: local-memory runtime dependency node-llama-cpp is missing (or failed to install)."
      : detail
        ? `Reason: ${detail}`
        : undefined,
    ...
    missing
      ? "2) Reinstall OpenClaw with npm or the installer script (this should restore node-llama-cpp): npm i -g openclaw@latest"
      : null,
