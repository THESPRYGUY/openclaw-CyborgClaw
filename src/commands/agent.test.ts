...
beforeEach(() => {
  ...
  vi.mocked(modelSelectionModule.resolveThinkingDefault).mockImplementation(({ catalog, model }) =>
    catalog?.some((entry) => entry.id === model && entry.reasoning) ? "medium" : "off",
  );
...
describe("agentCommand", () => {
  ...
  it("passes medium default thinking for gpt-5.2-codex to embedded runs", async () => {
    await withTempHome(async (home) => {
      const store = path.join(home, "sessions.json");
      mockConfig(home, store, {
      ...
      expect(getLastEmbeddedCall()?.thinkLevel).toBe("medium");
      expectLastRunProviderModel("openai-codex", "gpt-5.2-codex");
    });
  });
