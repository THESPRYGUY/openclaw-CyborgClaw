...
async function loadFreshHealthModulesForTest() {
  ...
  vi.doMock("../channels/plugins/read-only.js", async () => {
    const { listChannelPlugins } = await import("../channels/plugins/registry.js");
    return {
      listReadOnlyChannelPluginsForConfig: () => listChannelPlugins(),
    };
  });
